...
static volatile bool s_auto_reconnect_stop_requested;
...
static void ble_auto_reconnect_task(void *arg)
{
    ...
    for (;;) {
        if (!device_config_get_ble_autoconnect() ||
            s_auto_reconnect_stop_requested) {
            // Keep exactly one supervisor for the process lifetime. Destroying
            // and recreating it across a rapid auto-off/auto-on sequence can
            // either lose the new start request or delete a task inside a
            // NimBLE GAP call.
            vTaskDelay(pdMS_TO_TICKS(BLE_AUTO_RECONNECT_FAST_IDLE_MS));
        ...
        if (s_connected || s_state == BLE_STATE_CONNECTED) {
            // Keep one supervisor task alive for the whole auto-connect
            // session. Exiting here races a disconnect event that tries to
            // schedule the next supervisor while this handle is still set.
            vTaskDelay(pdMS_TO_TICKS(BLE_AUTO_RECONNECT_FAST_IDLE_MS));
            continue;
        }
...
static void schedule_auto_reconnect(const char *reason, uint32_t delay_ms)
{
    ...
    s_auto_reconnect_stop_requested = false;
                                    ...
                                    bool cancel_scan)
{
    ...
    s_state = BLE_STATE_STOPPING;
    APP_LOGI(TAG, "BLE selected target waits for DISC_COMPLETE before connect");
}
...
static int ble_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC_COMPLETE:
        ...
        APP_LOGI(TAG, "BLE scan complete reason=%d seen=%lu",
                 ...
                 (unsigned long)s_scan_seen_count);
        if (!s_connected &&
            (s_state == BLE_STATE_SCANNING || s_state == BLE_STATE_STOPPING)) {
            s_state = BLE_STATE_IDLE;
        }
...
static esp_err_t ble_central_connect_addr(const ble_addr_t *target, const char *label)
{
    ...
    if (ble_gap_disc_active()) {
        s_auto_scan_target = *target;
        snprintf(s_auto_scan_label, sizeof(s_auto_scan_label), "%s",
                 label ? label : "manual");
        s_auto_scan_target_valid = true;
        s_auto_scan_connect = false;
        s_auto_scan_preferred_valid = false;
        ...
        s_state = BLE_STATE_STOPPING;
        APP_LOGI(TAG, "BLE connect deferred until DISC_COMPLETE target=%s",
                 label ? label : "<addr>");
        return ESP_OK;
    }

    if (s_state == BLE_STATE_STOPPING ||
        s_state == BLE_STATE_CONNECTING ||
        ble_gap_conn_active()) {
        APP_LOGW(TAG, "BLE connect deferred/rejected while GAP state=%s",
                 ble_central_state_string());
        return ESP_ERR_INVALID_STATE;
    }

    if (s_connected) {
        APP_LOGW(TAG, "BLE connect rejected while connected; disconnect first");
        return ESP_ERR_INVALID_STATE;
    }
                                                ...
                                                uint32_t duration_ms)
{
    ...
    if (s_state == BLE_STATE_STOPPING) {
        APP_LOGI(TAG, "BLE scan deferred until GAP stop completes");
        return ESP_ERR_INVALID_STATE;
    }

    if (ble_gap_disc_active()) {
        // Reuse the one active GAP discovery. Cancel+restart from another task
        // races NimBLE's host timer and can corrupt its mutex/callout objects.
        s_state = BLE_STATE_SCANNING;
        if (auto_connect) {
            s_auto_scan_connect = true;
            if (preferred) {
                s_auto_scan_preferred = *preferred;
                s_auto_scan_preferred_valid = true;
            }
        }
        APP_LOGI(TAG, "BLE active scan reused auto_connect=%s preferred=%s",
                 s_auto_scan_connect ? "yes" : "no",
...
void ble_central_disconnect(void)
{
    ...
    // Let the reconnect task leave its loop at a safe point. Deleting it while
    // it is entering a NimBLE GAP call can leave scan/connect state half-owned.
    s_auto_reconnect_stop_requested = true;
    bool gap_stop_pending = false;
    if (ble_gap_disc_active()) {
        int rc = ble_gap_disc_cancel();
        gap_stop_pending = true;
        APP_LOGI(TAG, "BLE discovery stop requested rc=%d", rc);
    }
    if (!s_connected && ble_gap_conn_active()) {
        int rc = ble_gap_conn_cancel();
        gap_stop_pending = true;
        APP_LOGI(TAG, "BLE connection stop requested rc=%d", rc);
    }
    if (s_connected) {
        int rc = ble_gap_terminate(s_conn_handle, BLE_ERR_REM_USER_CONN_TERM);
        gap_stop_pending = true;
        APP_LOGI(TAG, "BLE link terminate requested handle=%u rc=%d", s_conn_handle, rc);
    }
    s_state = gap_stop_pending ? BLE_STATE_STOPPING : BLE_STATE_IDLE;
    APP_LOGI(TAG, "BLE disconnect requested state=%s", ble_central_state_string());
}
...
void ble_central_get_conn_metrics(ble_central_conn_metrics_t *out_metrics)
{
    ...
    *out_metrics = s_conn_metrics;
    ...
    out_metrics->scanning = s_state == BLE_STATE_SCANNING ||
                            s_state == BLE_STATE_STOPPING ||
                            ble_gap_disc_active();
