...
static int16_t switch1_scale_gyro_raw(int16_t pro2_raw)
{
    // Both controllers use Nintendo physical axes. Only their gyro LSB differs:
    // Pro2 is 14.247 raw/dps, Switch 1 Pro is approximately 14.285 raw/dps.
    int32_t scaled = (int32_t)pro2_raw * 14285;
    scaled += scaled >= 0 ? 7123 : -7123;
    return clamp_i16(scaled / 14247);
}

void report_mapper_switch1_convert_motion_block(const uint8_t source[SWITCH2_MOTION_BLOCK_SIZE],
                                                uint8_t target[SWITCH2_MOTION_BLOCK_SIZE])
{
    if (!target) {
        return;
    }
    memset(target, 0, SWITCH2_MOTION_BLOCK_SIZE);
    if (!source) {
        return;
    }

    for (uint8_t offset = 0; offset < SWITCH2_MOTION_BLOCK_SIZE;
         offset += SWITCH2_MOTION_SAMPLE_SIZE) {
        // Acceleration is 4096 raw/g on both Nintendo paths; axes are already
        // in the same controller-local coordinate system.
        memcpy(target + offset, source + offset, 6);
        write_i16_le(target + offset + 6,
                     switch1_scale_gyro_raw(read_i16_le(source + offset + 6)));
        write_i16_le(target + offset + 8,
                     switch1_scale_gyro_raw(read_i16_le(source + offset + 8)));
        write_i16_le(target + offset + 10,
                     switch1_scale_gyro_raw(read_i16_le(source + offset + 10)));
    }
}
...
void report_mapper_state_to_switch1_pro_report(const switch2_state_t *state,
                                               bool imu_enabled,
                                               uint8_t report[SWITCH_LEGACY_REPORT_SIZE])
{
    internal_gamepad_state_t internal;
    ...
    if (!state || !imu_enabled || !state->motion_valid) {
        memset(report + 13, 0, SWITCH2_MOTION_BLOCK_SIZE);
        return;
    }

    report_mapper_switch1_convert_motion_block(state->motion, report + 13);
}
