...
esp_err_t control_protocol_handle_line(const char *line, char *reply, int reply_len)
{
    ...
    if (strcmp(cmd, "status") == 0 || strcmp(cmd, "status lite") == 0) {
        ...
        char switch1_rumble_json[512];
        char combined_status_json[3600];
        ...
        snprintf(switch1_rumble_json,
                 sizeof(switch1_rumble_json),
                 "\"switch1_vibration_command_seen\":%s,\"switch1_vibration_enabled\":%s,\"switch1_vibration_commands\":%lu,\"switch1_rumble_rx\":%lu,\"switch1_rumble_active\":%lu,\"switch1_rumble_neutral\":%lu,\"switch1_rumble_forwarded\":%lu,\"switch1_rumble_blocked\":%lu",
                 usb_hid_device_switch1_vibration_command_seen() ? "true" : "false",
                 usb_hid_device_switch1_vibration_enabled() ? "true" : "false",
                 (unsigned long)usb_hid_device_switch1_vibration_command_count(),
                 (unsigned long)usb_hid_device_switch1_rumble_rx_count(),
                 (unsigned long)usb_hid_device_switch1_rumble_active_count(),
                 (unsigned long)usb_hid_device_switch1_rumble_neutral_count(),
                 (unsigned long)usb_hid_device_switch1_rumble_forwarded_count(),
                 (unsigned long)usb_hid_device_switch1_rumble_blocked_count());
    ...
    if (strcmp(cmd, "mode switch1") == 0 ||
        strcmp(cmd, "mode switch1_pro") == 0) {
        esp_err_t err = device_config_save_mode(SWITCH1_PRO_MODE);
        if (err != ESP_OK) {
            return json_error(reply, reply_len, "mode", "failed to save Switch 1 Pro mode");
        }
        return json_ok(reply, reply_len, "mode", "\"mode\":\"switch1_pro\",\"saved\":true,\"experimental\":true,\"reboot_required\":true");
    }
    ...
    if (strcmp(cmd, "switch1 rumble status") == 0) {
        char extra[512];
        snprintf(extra,
                 sizeof(extra),
                 "\"vibration_command_seen\":%s,\"vibration_enabled\":%s,\"vibration_commands\":%lu,\"rx\":%lu,\"active\":%lu,\"neutral\":%lu,\"forwarded\":%lu,\"blocked\":%lu,\"ble_writes\":%lu,\"ble_errors\":%lu",
                 usb_hid_device_switch1_vibration_command_seen() ? "true" : "false",
                 usb_hid_device_switch1_vibration_enabled() ? "true" : "false",
                 (unsigned long)usb_hid_device_switch1_vibration_command_count(),
                 (unsigned long)usb_hid_device_switch1_rumble_rx_count(),
                 ...
                 (unsigned long)usb_switch2_vendor_hd_rumble_write_count(),
                 (unsigned long)usb_switch2_vendor_hd_rumble_error_count());
        return json_ok(reply, reply_len, "switch1 rumble status", extra);
    }
    if (strcmp(cmd, "switch1 rumble test") == 0) {
        usb_switch2_vendor_start_hd_rumble_self_test();
        return json_ok(reply, reply_len, "switch1 rumble test",
                       "\"rumble\":\"active\",\"mode\":\"pro2_ble_path_self_test\"");
    }
