...
$PackageVersion = "5.9.18-switch1-connect-rumble-test-r4"
...
$ReadmeSource = Join-Path $RepoRoot "tools\release_notes_v5_9_18_switch1_test.zh-CN.md"
...
function Add-Switch1ProProfilePayload([string]$TargetRoot) {
    return Add-XInputBridgeProfilePayload `
        -TargetRoot $TargetRoot `
        -Profile "switch1_pro_bridge_v5_9_18_test" `
        -BuildDirRelative "work\b\switch1_pro_test" `
        -Label "Switch 1 / Pro wired-to-wireless test"
}
...
function Refresh-EmbeddedAssets {
    ...
    $profiles += Add-Switch1ProProfilePayload $firmwareRoot
    ...
    $manifest = [ordered]@{
        ...
        profiles = $profiles
        notes = "V5.9.18 Switch 1 connection and rumble test r4: fixes the r3 NimBLE GAP cancel/restart race with a single persistent reconnect supervisor and DISC_COMPLETE-owned transitions; keeps the rumble, VIIPER owner, and mode-detection fixes. Existing V5.9.17 profiles remain unchanged."
    }
...
if (!$SkipFirmwareBuild) {
    ...
    Write-Step "firmware_build" "switch1_pro_bridge_v5_9_18_test"
    if (!$DryRun) {
        & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $RepoRoot "tools\esp32s3\build.ps1") `
            -IdfPath $IdfPath `
            -BuildDir "work\b\switch1_pro_test" `
            -DeviceDefaultMode SWITCH1_PRO_MODE
        if ($LASTEXITCODE -ne 0) { throw "Firmware build failed: switch1_pro_bridge_v5_9_18_test" }
    }
...
if (!$DryRun) {
    ...
    $verifyLog = Join-Path $RepoRoot "work\v5_9_18_switch1_manager_package_verify.txt"
    ...
    if ($verifyData["result"] -ne "passed" -or
        $verifyData["profiles"] -ne "hid_audio_uac1_4ch_dualsense,hid_audio_uac1_4ch_edge,hid_only,pro2_bridge_v5_5,xinput_bridge_v5_8,switch1_pro_bridge_v5_9_18_test,dual_pro2_probe_v5_9" -or
        $verifyData["switch1_assets"] -ne "3" -or
        $verifyData["dual_probe_assets"] -ne "3" -or
