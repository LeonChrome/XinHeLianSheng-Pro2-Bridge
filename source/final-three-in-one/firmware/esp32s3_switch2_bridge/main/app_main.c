...
static void hid_report_task(void *arg)
{
    ...
    while (true) {
        ...
        /*
         * A short notification gap is not a button release. Keep the most
         * recent controller state until the BLE layer explicitly clears it
         * on disconnect/reset/recovery. Falling back to neutral solely
         * because a notification is late creates a false release followed by
         * a false press, which hosts surface as double-click or turbo input.
         */
        bool using_live = live_valid;
            ...
            if (err == ESP_OK) {
                ...
                if (now_us >= next_log_us) {
                    APP_LOGI(TAG, "report loop usb_mode=%s source=%s cadence=%s configured_test_rate_hz=%u live_updates=%lu live_age_ms=%lld test_mode=%s test_a=%s",
                             device_mode_to_string(device_config_get_mode()),
                             source,
                             using_live ? "ble_source_updates_hold_last_state" :
...
{
    app_log_init();
    APP_LOGI(TAG, "ESP32-S3 Switch 2 bridge firmware 5.9.19 starting");
    APP_LOGI(TAG, "Input cadence: latest-state BLE notification paced; hold last state across notification gaps; neutral only on explicit source loss");
