...
type DualSense struct {
	...
	rawOutputFunc     func([]byte)
	hapticAudioFunc   func([]byte)
	descriptor        usb.Descriptor
	hapticEnabled     bool
	audioStreaming    bool
	hapticAudioFrames uint64
...
func (d *DualSense) SetRawOutputCallback(f func([]byte)) {
	d.mtx.Lock()
	defer d.mtx.Unlock()
	d.rawOutputFunc = f
}

func (d *DualSense) SetHapticAudioCallback(f func([]byte)) {
	d.mtx.Lock()
	defer d.mtx.Unlock()
	d.hapticAudioFunc = f
}
...
func (d *DualSense) HandleTransfer(ctx context.Context, ep uint32, dir uint32, out []byte) []byte {
	...
	if dir == usbip.DirOut && ep == 3 {
		if len(out) >= 48 && out[0] == ReportIDOutput {
			...
			d.mtx.Lock()
			rawCallback := d.rawOutputFunc
			d.mtx.Unlock()
			if rawCallback != nil {
				rawCallback(append([]byte(nil), out...))
			}
		}
	}

	if dir == usbip.DirOut && ep == 1 && d.hapticEnabled {
		d.mtx.Lock()
		audioCallback := d.hapticAudioFunc
		d.hapticAudioFrames++
		frameCount := d.hapticAudioFrames
		d.mtx.Unlock()
		if frameCount == 1 {
			slog.Info(
				"DualSense haptic audio callback input is live",
				"bytes", len(out),
				"callbackReady", audioCallback != nil,
			)
		}
		if audioCallback != nil && len(out) > 0 {
			audioCallback(append([]byte(nil), out...))
		}
	}
...
func (d *DualSense) HandleControl(bmRequestType, bRequest uint8, wValue, wIndex, wLength uint16, data []byte) ([]byte, bool) {
	if d.hapticEnabled {
		...
		case bmRequestType == 0x01 && bRequest == 0x0b:
			...
			if iface == 1 {
				d.mtx.Lock()
				d.audioStreaming = alt != 0
				d.mtx.Unlock()
			}
			return nil, true
		case bmRequestType == 0xa2 && bRequest == 0x81 && uint8(wValue>>8) == 0x01:
			return []byte{0x80, 0xbb, 0x00}, true
		case bmRequestType == 0x22 && bRequest == 0x01:
			return nil, true
		case bmRequestType == 0xa1:
			switch bRequest {
			case 0x81:
				return make([]byte, wLength), true
			case 0x82:
				return fitControlResponse([]byte{0x00, 0xa0}, wLength), true
			case 0x83:
				return fitControlResponse([]byte{0x00, 0x00}, wLength), true
			case 0x84:
				return fitControlResponse([]byte{0x00, 0x01}, wLength), true
			}
		case bmRequestType == 0x21 && bRequest == 0x01:
			return nil, true
		case bmRequestType == 0x02 && bRequest == 0x01:
			return nil, true
		}
	}
	...
	case hidClassOUT:
		...
		case hidSetIdle, hidSetProtocol:
			return nil, true
		}
	}
...
func fitControlResponse(value []byte, length uint16) []byte {
