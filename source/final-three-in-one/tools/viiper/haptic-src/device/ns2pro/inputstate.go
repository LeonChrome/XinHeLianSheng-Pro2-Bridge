...
// nolint
// viiper:wire ns2pro c2s buttons:u32 lx:u16 ly:u16 rx:u16 ry:u16 accelX:i16 accelY:i16 accelZ:i16 gyroX:i16 gyroY:i16 gyroZ:i16 motionTimestampUs:u32
type InputState struct {
	...
	// MotionTimestampUs is the BLE source clock carried over the VIIPER wire
	// for ordering and telemetry. The virtual USB endpoint generates its own
	// monotonic motion clock for every final report.
	MotionTimestampUs uint32
}
...
func (s *InputState) MarshalBinary() ([]byte, error) {
	...
	binary.LittleEndian.PutUint32(b[24:28], s.MotionTimestampUs)
	return b, nil
}
...
func (s *InputState) UnmarshalBinary(data []byte) error {
	...
	s.MotionTimestampUs = binary.LittleEndian.Uint32(data[24:28])
	return nil
}
