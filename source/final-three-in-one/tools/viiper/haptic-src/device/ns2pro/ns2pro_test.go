...
func TestBuildReport05(t *testing.T) {
	...
	assert.Equal(t, uint32(4), binary.LittleEndian.Uint32(report[1:5]))
	...
	assert.Equal(t, uint32(4_000), ts1)
	...
	assert.Less(t, time.Since(start), 10*time.Millisecond, "enabled input endpoint must not add a second bInterval wait")
	assert.Equal(t, uint32(8), binary.LittleEndian.Uint32(next[1:5]))
	assert.Equal(t, uint32(8_000), binary.LittleEndian.Uint32(next[0x2B:0x2F]))
}

func TestInputStateWirePreservesSourceTimestampButEndpointOwnsMotionClock(t *testing.T) {
	state := InputState{
		LX:                StickCenter,
		LY:                StickCenter,
		RX:                StickCenter,
		RY:                StickCenter,
		GyroX:             123,
		MotionTimestampUs: 0x12345678,
	}
	wire, err := state.MarshalBinary()
	require.NoError(t, err)
	require.Len(t, wire, InputWireSize)
	assert.Equal(t, uint32(0x12345678), binary.LittleEndian.Uint32(wire[24:28]))

	var decoded InputState
	require.NoError(t, decoded.UnmarshalBinary(wire))
	assert.Equal(t, state.MotionTimestampUs, decoded.MotionTimestampUs)

	dev, err := New(nil)
	require.NoError(t, err)
	dev.UpdateInputState(decoded)
	dev.HandleTransfer(context.Background(), 2, usbip.DirOut, featureCommand(0x02, FeatureIMU))
	dev.HandleTransfer(context.Background(), 2, usbip.DirOut, featureCommand(0x04, FeatureIMU))
	dev.HandleTransfer(context.Background(), 2, usbip.DirOut, selectReportCommand(ReportIDCommon))
	dev.HandleTransfer(context.Background(), 2, usbip.DirOut, enableReportsCommand())
	report := dev.HandleTransfer(context.Background(), 1, usbip.DirIn, nil)
	require.Len(t, report, InputReportSize)
	finalTimestamp := binary.LittleEndian.Uint32(report[0x2B:0x2F])
	assert.NotEqual(t, state.MotionTimestampUs, finalTimestamp)
...
func TestInputEndpointPublishesLatestStateWithoutBurstReplay(t *testing.T) {
	...
	states := []InputState{
		{GyroZ: 101, MotionTimestampUs: 15_000},
		{GyroZ: 202, MotionTimestampUs: 30_000},
		{GyroZ: 303, MotionTimestampUs: 45_000},
	}
...
func TestDeviceSpecificInputInterval(t *testing.T) {
	dev, err := New(&device.CreateOptions{
		DeviceSpecific: `{"input_interval_ms":15}`,
	})
	require.NoError(t, err)
	require.NotEmpty(t, dev.GetDescriptor().Interfaces)
	require.Len(t, dev.GetDescriptor().Interfaces[0].Endpoints, 2)
	assert.Equal(t, uint8(15), dev.GetDescriptor().Interfaces[0].Endpoints[0].BInterval)
	assert.Equal(t, uint8(4), dev.GetDescriptor().Interfaces[0].Endpoints[1].BInterval)
}

func TestCommonReportClocksFollowConfiguredEndpointInterval(t *testing.T) {
	dev, err := New(&device.CreateOptions{
		DeviceSpecific: `{"input_interval_ms":15}`,
	})
	require.NoError(t, err)
	...
	require.Len(t, first, InputReportSize)
	require.Len(t, second, InputReportSize)
	assert.Equal(t, uint32(15), binary.LittleEndian.Uint32(first[1:5]))
	assert.Equal(t, uint32(15_000), binary.LittleEndian.Uint32(first[0x2B:0x2F]))
	assert.Equal(t, uint32(30), binary.LittleEndian.Uint32(second[1:5]))
	assert.Equal(t, uint32(30_000), binary.LittleEndian.Uint32(second[0x2B:0x2F]))
}
