...
public static class DeviceInspector
{
    ...
    private static string QueryUsbSummary()
    {
        ...
        using var searcher = new ManagementObjectSearcher(
            "SELECT Name,Status,PNPClass,DeviceID,Present,ConfigManagerErrorCode FROM Win32_PnPEntity");
        foreach (ManagementObject item in searcher.Get())
        {
            bool present = item["Present"] is not bool presentValue || presentValue;
            uint errorCode = Convert.ToUInt32(item["ConfigManagerErrorCode"] ?? 0u);
            if (!present || errorCode != 0)
            {
                continue;
            }
            ...
            else if (deviceId.Contains("VID_CAFE&PID_4037", StringComparison.OrdinalIgnoreCase))
            {
                identity = "macOS / Standard HID VID_CAFE&PID_4037";
            }
            else
            {
                continue;
        ...
        if (lines.Count == 0)
        {
            return "没有检测到新和联胜 / PS5 Edge 054C:0DF2、兼容 PS5 054C:0CE6、Nintendo/Pro2 057E:2069、Xbox/XInput 045E:028E 或 macOS 标准 HID CAFE:4037 设备。如果刚刷完固件，请重新插拔原生 USB / OTG。";
        }
